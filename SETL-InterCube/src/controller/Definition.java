...
public class Definition {
	...
	private LinkedHashMap<String, ArrayList<String>> selectedMeasureFunctionMap1;
	private ArrayList<String> allCubeLevels;
	private ArrayList<SelectedLevel> selectedLevelList1;
	...
	public void initializeOlapEndPoint(String endPoint) {
		...
		setSelectedLevelList1(new ArrayList<>());
		setInstancesMap(new LinkedHashMap<>());
		setSelectedMeasureFunctionMap(new LinkedHashMap<>());
		setSelectedMeasureFunctionMap1(new LinkedHashMap<>());
	...
	private void setSelectedMeasureFunctionMap1(LinkedHashMap<String, ArrayList<String>> selectedMeasureFunctionMap) {
		// TODO Auto-generated method stub
		this.selectedMeasureFunctionMap1 = selectedMeasureFunctionMap;
	}
	
	public LinkedHashMap<String, ArrayList<String>> getSelectedMeasureFunctionMap1() {
		return selectedMeasureFunctionMap1;
	}
	...
	public ArrayList<SelectedLevel> getSelectedLevelList1() {
		return selectedLevelList1;
	}

	public void setSelectedLevelList1(ArrayList<SelectedLevel> selectedLevelList) {
		this.selectedLevelList1 = selectedLevelList;
	}
	...
	public String generateOlapQuery(String datasetName, ArrayList<String> bannedLevels) {
		...
		int count = 1;
		for (String measureName : getSelectedMeasureFunctionMap().keySet()) { //getSelectedMeasureFunctionMap provides measure list
		...
		for (SelectedLevel selectedLevel : getSelectedLevelList()) {
			LinkedHashMap<String, String> levelQueryMap = generateLevelOlapQuery(selectedLevel, bannedLevels, o);
	...
	public String generateOlapQueryForTwoCube(String dataset1Name, String dataset2Name, ArrayList<String> bannedLevelsFromCube1, ArrayList<String> bannedLevelsFromCube2) {
		// TODO Auto-generated method stub

		dataset1Name = extraction.assignIRI(dataset1Name);
		dataset2Name = extraction.assignIRI(dataset2Name);
		...
		for (SelectedLevel selectedLevel : selectedLevelList1) {
			String tagString = selectedLevel.getLevelName();
			String levelPath = selectedLevel.getLevelPath();
		...
		for (String measureName : getSelectedMeasureFunctionMap().keySet()) {
			...
			for (String aggregateFunction : aggregateFunctions) {
				...
				String functionQuery = "(" + functionName.toUpperCase() + "(<" + measureRangeString + ">(?m" + count
						+ ")) as ";
				String functionTag = "?" + methods.getLastSegment(measureName) + "_" + functionName.toLowerCase() + "_" + methods.getLastSegment(dataset1Name);
				functionQuery += functionTag + ") ";
				measureTagString += functionQuery;
				selectedColumns.add(functionTag);
			}
		...
		count = 1;
		for (String measureName : getSelectedMeasureFunctionMap1().keySet()) {
			String measureRangeString = extraction.getOlapMeasureRange(endpointString, selectedGraphs, measureName);

			// GET ALL MEASURE TAGS
			ArrayList<String> aggregateFunctions = getSelectedMeasureFunctionMap1().get(measureName);
			String measureTagString = "";

			for (String aggregateFunction : aggregateFunctions) {
				String functionName = methods.getLastSegment(aggregateFunction);
				String functionQuery = "(" + functionName.toUpperCase() + "(<" + measureRangeString + ">(?n" + count
						+ ")) as ";
				String functionTag = "?" + methods.getLastSegment(measureName) + "_" + functionName.toLowerCase() + "_" + methods.getLastSegment(dataset2Name);
				functionQuery += functionTag + ") ";
				measureTagString += functionQuery;
				selectedColumns.add(functionTag);
			}
		...
		queryString += "{\n";
		queryString += "?" + o + " a qb:Observation .\n";
		queryString += "?"  + o + " qb:dataSet <" + dataset1Name + "> .\n";
		...
		LinkedHashMap<String, String> queryMap = new LinkedHashMap<>();
		for (SelectedLevel selectedLevel : getSelectedLevelList()) {
			LinkedHashMap<String, String> levelQueryMap = generateLevelOlapQuery(selectedLevel, bannedLevelsFromCube1, o);
		...
		String p = "p";
		queryString += "{\n";
		queryString += "?" + p + " a qb:Observation .\n";
		queryString += "?" + p + " qb:dataSet <" + dataset2Name + "> .\n";

		count = 1;
		for (String measureName : getSelectedMeasureFunctionMap1().keySet()) {
			queryString += "?"  + p + " <" + extraction.assignIRI(measureName) + "> ?n" + count + " .\n";
			count++;
		}

		// GET ALL LEVELS STRING
		LinkedHashMap<String, String> queryMap1 = new LinkedHashMap<>();
		for (SelectedLevel selectedLevel : getSelectedLevelList1()) {
			LinkedHashMap<String, String> levelQueryMap = generateLevelOlapQuery(selectedLevel, bannedLevelsFromCube2, p);

			for (String levelQuery : levelQueryMap.keySet()) {
				queryMap1.put(levelQuery, levelQuery);
			}
		}

		for (String levelQuery : queryMap1.keySet()) {
			queryString += levelQuery;
		}
	...
	private LinkedHashMap<String, String> generateLevelOlapQuery(SelectedLevel selectedLevel, ArrayList<String> bannedLevels, String s) {
		// TODO Auto-generated method stub
		...
		if (hierLevels == null || hierLevels.size() == 1) {
			String queryString = "?" + s + " <" + selectedLevelString + "> " + levelTagKey + " .\n";
					...
					if (!checkedList.contains(levelString)) {
						if (checkedList.size() == 0) {
							...
							if (bannedLevels.contains(extraction.assignPrefix(childLevel))) {
								String queryString = "?" + s + " <" + parentLevel + "> " + parentTag + " .\n";
								levelQueryMap.put(queryString, queryString);
								...
								String queryString = "?" + s + " <" + childLevel + "> " + childTag + " .\n";
								levelQueryMap.put(queryString, queryString);
