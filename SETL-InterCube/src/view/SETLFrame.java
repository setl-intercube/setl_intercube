...
public class SETLFrame extends JFrame {
	...
	public SETLFrame() {
		...
		JMenuItem mntmOlapLayerForTwoCube = new JMenuItem("OLAP Layer For Two Cube");
		mntmOlapLayerForTwoCube.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// To add a panel as Tab, you only need to create an object of that panel
				// and pass it to the following method as parameter -- Amrit
				...
				for (TabModel tabModel : tabModelList.getTabModels()) {
					if (tabModel.getTabObject() instanceof PanelOlap2) {
						count++;
					}
				}
				
				PanelOlap2 panelOlapNew = new PanelOlap2();
				tabModelList.getTabModels().add(new TabModel(tabbedPaneContainer.getTabCount(), panelOlapNew));
				addPanelToContainer("OLAPTwo" + count, panelOlapNew);
			}
		});
		mnNew.add(mntmOlapLayerForTwoCube);
